  ...
  return (
    <div className="mb-4">
      <div className="flex justify-between items-center mb-1.5">
        <span id={id} className="text-sm font-medium text-primary">
          {name}
        </span>
        <span className="text-sm md:text-xs font-sans text-secondary" aria-hidden="true">
          {proficiency}%
        </span>
      </div>
      <div
      ...
      >
        <motion.div
          ...
          initial={shouldReduceMotion ? { width: `${proficiency}%` } : { width: 0 }}
          animate={shouldReduceMotion ? { width: `${proficiency}%` } : undefined}
          whileInView={shouldReduceMotion ? undefined : { width: `${proficiency}%` }}
